...
const upstream = http.createServer(async (req, res) => {
  ...
  const babyPath = `/api/v1/babies/${baby.id}`;
  const recordLists = ['feeding', 'sleep', 'diaper', 'food', 'supplement'].map(kind => `${babyPath}/records/${kind}`);
  if (recordLists.includes(url.pathname) || url.pathname === `${babyPath}/growth-measurements`) {
    res.end(JSON.stringify({ data: [], page: { nextCursor: null } })); return;
  }
  if ([`${babyPath}/vaccines/records`, `${babyPath}/vaccines/schedule`].includes(url.pathname)) { send([]); return; }
  if (url.pathname === `${babyPath}/food-plan`) { send({ id: null, babyId: baby.id, planData: {}, version: '0', createdAt: null, updatedAt: null }); return; }
  if (url.pathname === '/api/v1/development/milestones') {
    res.end(JSON.stringify({ data: [], dataRelease: { id: 'test_release', title: 'Test reference release', asOf: '2026-01-01', sources: [{ organization: 'Test fixture' }] } })); return;
  }
...
try {
  ...
  console.log('PASS: actual Next HTTP Go-backend selector, login/cookie/me/baby/feeding/notifications, CSRF, outage and logout wiring. Synthetic upstream; no PostgreSQL or browser assertions.');
} catch (error) {
