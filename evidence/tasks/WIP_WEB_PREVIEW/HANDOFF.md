...
- 新增知识、绘本、天气、附件、医疗详情BFF及部分允许清单；医疗items/attachment ID/原版本保留。
- 账号代理新增register、family preview/join/member/invite、session/identity、store切换和家庭页代码，任务被用户暂停时仍在整合，未完成独立验收。
- 记录代理新增record-list/record-route-helpers/record-compat-helpers、日期/分页、详情编辑及日报聚合代码，测试仍有失败。
- `stores/useBabyStore.ts` 与records slice的saveBaby avatarUrl nullable类型不一致。新增记录单测存在DatedRecord.id与泛型测试类型错误；见typecheck.log。
- 日期测试实际错误包含越界feeding记录和睡眠边界记录；先检查时区与区间边界，见record-tests.log。
- `lib/mcp/server.ts` 的 `_options` 目前仅为保留合并后签名，新模式不能据此使用旧实现。
