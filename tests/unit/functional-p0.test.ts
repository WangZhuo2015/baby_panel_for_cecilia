...
test("TDD: GrowthMeasurement should have clientId for offline idempotency (B2)", () => {
  ...
  // Check store has offline handling
  const store = fs.readFileSync(path.join(process.cwd(), "stores/useBabyStore.ts"), "utf-8");
  assert.ok(store.includes("addGrowthMeasurement") && store.includes("clientId"), "store should handle clientId for growth");
});

test("TDD: GET /api/growth should use desc order not asc (B3)", () => {
  const content = fs.readFileSync(path.join(process.cwd(), "app/api/growth/route.ts"), "utf-8");
  assert.ok(content.includes('orderBy'), "has orderBy");
  // After fix, should be desc to return latest, not asc
  assert.ok(content.includes('"desc"') || content.includes("'desc'") || content.includes("desc"), "should use desc");
  // Ensure not purely asc without limit handling
...
test("TDD: feeding should guard future timestamp (B9)", () => {
  const content = fs.readFileSync(path.join(process.cwd(), "app/api/records/feeding/route.ts"), "utf-8");
