  ...
  try {
    if (GROWDESK_CONFIG.enabled) {
      ...
      const res = await growdeskFetch<GrowDeskDiaperRecord>(recordPath("diaper", babyId), {
        ...
        body: toGrowDeskDiaperCreatePayload(body),
      });
      const data = requireWriteData(res, "Failed to create diaper record");
      return NextResponse.json(fromGrowDeskDiaperRecord(data), { status: 201 });
    }
    const auth = await requireAuth(request);
    if (auth.errorResponse) return auth.errorResponse;
...
export async function DELETE(request: Request) {
  try {
    if (GROWDESK_CONFIG.enabled) {
      ...
      const res = await growdeskFetch(recordPath("diaper", babyId, id) + `?baseVersion=${encodeURIComponent(version)}`, {
        method: "DELETE",
        accessToken: bffSession.accessToken,
        idempotencyKey: idempotencyKey(body, request),
      });
      if (!res.ok) return NextResponse.json({ error: res.error?.message || "Failed to delete diaper record" }, { status: res.status });
...
export async function PUT(request: Request) {
  try {
    if (GROWDESK_CONFIG.enabled) {
      ...
      if (typeof babyId !== "string" || !babyId) return NextResponse.json({ error: "请提供 babyId" }, { status: 400 });
      const res = await growdeskFetch<GrowDeskDiaperRecord>(recordPath("diaper", babyId, id), {
        method: "PATCH",
        accessToken: bffSession.accessToken,
        idempotencyKey: idempotencyKey(body, request),
        body: toGrowDeskDiaperUpdatePayload(body),
      });
      const data = requireWriteData(res, "Failed to update diaper record");
      return NextResponse.json(fromGrowDeskDiaperRecord(data));
    }
    const auth = await requireAuth(request);
    if (auth.errorResponse) return auth.errorResponse;
    const body = await request.json().catch(() => ({} as any));
