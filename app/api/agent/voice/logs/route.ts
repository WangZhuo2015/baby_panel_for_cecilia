...
import { listGrowDeskVoiceLogs } from "@/lib/growdesk/voice-log-api";
...
export async function GET(request: Request) {
  try {
    if (GROWDESK_CONFIG.enabled) {
      ...
      const res = await listGrowDeskVoiceLogs(growdeskFetch, bffSession.accessToken, { limit, unreadAsyncOnly });
      if (unreadAsyncOnly) {
