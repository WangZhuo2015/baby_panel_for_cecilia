...
export async function GET(request: Request) {
  try {
    if (GROWDESK_CONFIG.enabled) {
      ...
      const res = await growdeskFetch<Array<GrowDeskFoodRecord>>(
        `/api/v1/babies/${babyId}/records/food?limit=${limit}`,
        {
          method: "GET",
      ...
      if (!res.ok) {
        return NextResponse.json(
          { error: res.error?.message || "Failed to fetch food log records" },
          { status: res.status },
        );
      }
      const rawList = Array.isArray(res.data) ? res.data : (res.data as any)?.data || [];
      return NextResponse.json(rawList.map(fromGrowDeskFoodRecord));
    }
...
export async function POST(request: Request) {
  try {
    if (GROWDESK_CONFIG.enabled) {
      ...
      const payload = toGrowDeskFoodCreatePayload(body);
      const idempotencyKey =
        body.clientId || request.headers.get("idempotency-key") || crypto.randomUUID();

      const res = await growdeskFetch<GrowDeskFoodRecord>(
      ...
      if (!res.ok || !res.data) {
        return NextResponse.json(
          { error: res.error?.message || "Failed to create food log record" },
          { status: res.status },
        );
      }

      return NextResponse.json(fromGrowDeskFoodRecord(res.data), { status: 201 });
    }
  ...
  try {
    if (GROWDESK_CONFIG.enabled) {
      ...
      const res = await growdeskFetch(
        `/api/v1/babies/${babyId}/records/food/${id}?baseVersion=${baseVersion}`,
        {
          method: "DELETE",
          accessToken: bffSession.accessToken,
        },
      ...
      if (!res.ok) {
        ...
        return NextResponse.json(
          { error: res.error?.message || "Failed to delete food log" },
          { status: res.status },
        );
      }
...
export async function PUT(request: Request) {
  try {
    if (GROWDESK_CONFIG.enabled) {
      ...
      const payload = toGrowDeskFoodUpdatePayload(body);
      const res = await growdeskFetch<GrowDeskFoodRecord>(
      ...
      if (!res.ok || !res.data) {
        return NextResponse.json(
          { error: res.error?.message || "Failed to update food log" },
          { status: res.status },
        );
      }

      return NextResponse.json(fromGrowDeskFoodRecord(res.data));
    }
